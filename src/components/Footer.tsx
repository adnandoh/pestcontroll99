...
/** Brand glyphs + hover colour per profile, keyed to SOCIAL_LINKS. */
const SOCIAL_ICONS: Record<string, { hoverClass: string; path: string }> = {
  youtube: {
  ...
  instagram: {
    hoverClass: 'hover:bg-[#d6249f] hover:border-[#d6249f]',
        ...
        return (
          <li key={profile.key}>
            <a
              ...
              aria-label={`${BUSINESS.brandName} on ${profile.label}`}
              title={profile.label}
              className={`flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/5 text-white/85 transition-colors hover:text-white ${icon.hoverClass}`}
            >
