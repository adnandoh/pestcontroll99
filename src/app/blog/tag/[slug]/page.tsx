...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: `Blogs tagged "${tag.name}" | PestControl99`,
    description: `All pest control blogs tagged ${tag.name}.`,
    alternates: {
      canonical: `https://www.pestcontrol99.com/blog/tag/${tag.slug}/`,
    },
    robots: {
      index: false,
      follow: true,
...
export default async function TagPage({
  ...
  const [blogsData, tags] = await Promise.all([
    getBlogs({ page, page_size: 12, tag: tagSlug }),
    getTags()
  ]);
    ...
    <div className="min-h-screen bg-gray-50">
      <Breadcrumb 
        items={[
          ...
          { label: `Tag: ${tag ? tag.name : tagSlug}` }
        ]} 
      />
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-4">Blogs tagged: &quot;{tag ? tag.name : tagSlug}&quot;</h1>
            </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center mt-12 gap-2">
                {page > 1 && (
                  <Link href={`/blog/tag/${tagSlug}?page=${page - 1}`} className="px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                    Previous
                  </Link>
                )}
